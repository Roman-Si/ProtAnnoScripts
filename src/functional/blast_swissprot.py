...
def parse_sprot_fasta(sprot_fasta_file):
    """
    ...
    with opener(sprot_fasta_file, "rt") as fall:
        ...
        for record in records:
            proteinIDs.append(record.id.split("|")[1] if '|' in record.id else record.id)

            description = record.description.replace(record.id, '')

            protein_names.append(re.match(r"(.*) OS\=.*", description).group(1))
            genes.append(re.match(r".* GN\=(.*) PE", description).group(1) if " GN=" in description else None)
            
            taxIDs.append(re.match(r".* OX\=([0-9]*) ", description).group(1) if " OX=" in description else None)
        
    df = pd.DataFrame({'sprotId': proteinIDs, 'Protein name': protein_names, "Gene": genes, "Taxon Id" : taxIDs})
    return(df)




def blast_sprot(blast_output, sprot_df, pident = 70, qcovs = 70, length_difference = 20):
    """
    Steps:
    ...
    # filter
    df = df.loc[(df['pident'] > pident) & (df['qcovs'] > qcovs) & (abs(df['qlen'] - df['slen']) / df['qlen'] < length_difference)]
    # keep best hit based on bitscore
    df = df.sort_values(by=['qacc', 'bitscore'], ascending=[True, False]).groupby('qacc').head(1)
    # merge with sprot_df to get protein annotations
    df = pd.merge(df, sprot_df, left_on = 'sacc', right_on = 'sprotId', how = 'left')
    df = df[['qacc', 'sprotId', 'Protein name', "Gene", "Taxon Id", 'pident', 'qcovs', 'scovs', 'evalue', 'bitscore']]
